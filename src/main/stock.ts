 ...
 *  and export offline.
 *
 *  `parseSvg` (the pure SVG→recolourable-paths parse) lives in
 *  `src/shared/stockParse.ts` — the native shell runs the same code in the
 *  renderer, so both hosts agree on vector data byte-for-byte.
 */
